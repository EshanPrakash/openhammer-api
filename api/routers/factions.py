...
router = APIRouter(prefix="/v1/{edition}/factions", tags=["Factions"])


@router.get("/{faction_name}/details")
async def get_faction_details(edition: str, faction_name: str):
...
async def get_faction_stats(edition: str, faction_name: str):
    store = data_store.get(edition)
    if not store:
        raise HTTPException(status_code=404, detail=f"Edition '{edition}' not found")
...
async def get_faction_keywords(edition: str, faction_name: str):
    store = data_store.get(edition)
    if not store:
        raise HTTPException(status_code=404, detail=f"Edition '{edition}' not found")
    ...
    return {
        ...
        "keywords": [{"keyword": k, "unit_count": v} for k, v in sorted_keywords]
    }
